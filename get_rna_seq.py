...
rrna_file = open('rrna.csv', 'w')
rrna_file.write(f'accn,taxon,species,genome_size_(mb),num_rrna\n')
...
for fl in files_all_short:
    ...
    del f_r
    rrna_list = gene_annot.split('FT   rRNA            ')
    print(len(rrna_list))
    rrna_list = rrna_list[1:]
    #print(rrna_list[0])
    ...
    for gn in rrna_list:
        ...
        f_w = open(f"{accn}/{lt1}.rrna", "w")
        f_w.write(f'>{taxon}:{species}:{lt1}:{tt1}\n{cds}')
        f_w.close()
    rrna_file.write(f'{accn},{taxon},{species},{(len(seq))/1000000},{len(rrna_list)}\n')
